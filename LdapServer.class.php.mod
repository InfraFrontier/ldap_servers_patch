 ...
 */
  ...
  public function createLdapEntry($attributes, $dn = NULL) {
    ...
    // Patching user password attribute 'unicodePwd' to work with Active Directory
    // as userPassword works for openLdap but not AD
    // The 128-bit encryption for password is also required
    foreach ($attributes as $key => $cur_val) {        
        if ($key == "unicodePwd") {           

            $newPassword = "\"" . $cur_val[0] . "\"";
            $len = strlen( $newPassword );
            $newPassw = "";
            for ( $i = 0; $i < $len; $i++ ){
                $newPassw .= "{$newPassword{$i}}\000";
            }
            $attributes[$key][0] = $newPassw;
        }  
    } 
